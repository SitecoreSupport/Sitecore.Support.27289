    ...
    var CommerceBasePage = Sitecore.Definitions.App.extend({
        ...
        initialized: function () {
           ...
           this.EnvironmentsSwitcher.on("change:selectedItem", this.onSelectedEnvironmentChanged, this);
            ...
            // Used to determine whether to display Sitecore Commerce related controls
           this.setSitecoreCommerceControls();
        },
...
//Fix for 27289
            $.ajax({
                url: "/sitecore/shell/commerce/merchandising/FixedCommercePricing/UpdateListPriceForItem",
                type: "POST",
        ...
        isCommerceFoundationEnabled: function() {
            var attr = $("body").attr("data-sc-commerceFoundationEnabled");
            return attr == "true";
        },

        setSitecoreCommerceControls: function () {
            var useSitecoreCommerce = this.isCommerceFoundationEnabled();

            if (useSitecoreCommerce) {
                // Initialize the EnvironmentsDataSource only if CF is enabled
                this.EnvironmentsDataSource.set("isReady", true);
                this.EnvironmentsDataSource.refresh();
            }

            if (this.EnvironmentsSwitcher) {
                this.EnvironmentsSwitcher.set("isVisible", useSitecoreCommerce);
            }

            if (this.PricingExpander) {
                this.PricingExpander.set("isVisible", useSitecoreCommerce);
            }

            if (this.PriceCardsComboBox) {
                this.PriceCardsComboBox.set("isVisible", useSitecoreCommerce);
                this.PriceCardLabelText.set("isVisible", useSitecoreCommerce);
            }

            if (this.ProductTabs) {
                var tab = $("li[data-tab-id='{26213DC0-B3F1-4682-B0D2-B6E87B9216B4}']", this.tabStrip);
            ...
            if (this.VariantPricingExpander) {
                this.VariantPricingExpander.set("isVisible", useSitecoreCommerce);
            }
            
            if (this.VariantPriceCardsComboBox) {
                this.VariantPriceCardsComboBox.set("isVisible", useSitecoreCommerce);
                this.VariantPriceCardLabelText.set("isVisible", useSitecoreCommerce);
            }

            if (this.VariantTab) {
                var tab = $("li[data-tab-id='{B8ED623C-9537-4CA4-89E4-669E03F8D233}']", this.tabStrip);
